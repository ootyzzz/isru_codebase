...
def run_default_simulation_with_visualization(args):
    ...
    # 运行策略对比 (T=10, 三个策略)
    strategies = ["upfront_production", "gradual_production", "flexible_production"]
...
def run_strategy_comparison(args):
    ...
    strategies = args.strategies if args.strategies else ["upfront_production", "gradual_production", "flexible_production"]
...
def compare_with_optimal(args):
    ...
    strategy_results = runner.run_strategy_comparison(
        strategies=args.strategies or ["upfront_production", "gradual_production", "flexible_production"],
        T=args.time_horizon,
        n_simulations=args.n_simulations,
        base_seed=args.seed,
        save_results=False,
...
def main():
    ...
    single_parser.add_argument('--strategy', choices=['upfront_production', 'gradual_production', 'flexible_production'],
                              default='gradual_production', help='策略类型')
    ...
    mc_parser.add_argument('--strategy', choices=['upfront_production', 'gradual_production', 'flexible_production'],
                          default='gradual_production', help='策略类型')
    ...
    compare_parser.add_argument('--strategies', nargs='+',
                               choices=['upfront_production', 'gradual_production', 'flexible_production'],
                               help='要对比的策略')
